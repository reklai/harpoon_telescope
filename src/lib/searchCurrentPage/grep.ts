...
/** Start observing DOM mutations. Call on search overlay open. */
...
/** Stop observing and release cache. Call on search overlay close. */
...
function collectImages(): TaggedLine[] {
  ...
  for (const img of images) {
    ...
    if (!isVisible(el)) continue;
    // Prefer alt text, fall back to title, then filename from src
